...
const {
  getUrl: oe_getUrl
} = __webpack_require__(/*! oe/modules/_config/key */ "./src/js/modules/_config/key.js");
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  keyInfo: keyInfo,
  //list, bmnet
  bm_modal_key: "bm.oe.modal",
  //list
  bm_creation_state: "bm.oe.creation",
  //bookmark
  bm_list_store: "bm.oe.list",
  //bmnet
  bm_topic_list: "bm.oe.topics",
  //bmnet
  bm_modal_store: "bm.oe.modal",
  //navigator
...
/* harmony import */ var _wbkey__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./wbkey */ "./src/js/modules/_config/wbkey.js");
...
const SOURCE_ID = "oe";
const SOURCE = "ACIM Original Edition"; //mp3 and audio timing base directories

const audioBase = `https://s3.amazonaws.com/${AWS_BUCKET}/${SOURCE_ID}/audio`;
const timingBase = "/t/acimoe/public/timing"; //location of configuration files

const configUrl = "/t/acimoe/public/config";
const configStore = "config.acimoe."; //the current configuration, initially null, assigned by getConfig()
...
function refreshNeeded(cfg) {
  ...
  if (cfg.saveDate === saveDate) {
    ...
    cfg.saveDate = saveDate; //console.log("%s needs to be refreshed", cfg.bid);
...
function getAudioInfo(url) {
  ...
  switch (idx[2]) {
    ...
    case "workbook":
      ...
      let index = Object(_wbkey__WEBPACK_IMPORTED_MODULE_3__["getIndexFromLesson"])(idx[3]);
      ...
      if (info.data) {
        ...
        */
        ...
        switch (decodedKey.bookId) {
          ...
          case "workbook":
            flat = store__WEBPACK_IMPORTED_MODULE_0___default.a.get(flat_store_id);

            if (!flat) {
              flat = flatten(data);
              store__WEBPACK_IMPORTED_MODULE_0___default.a.set(flat_store_id, flat);
            }
...
const status = {
  acq: "Thu Apr 30 13:20:22 HST 2020",
  manual: "Wed May 6 10:47:44 HST 2020",
  text: "Sat May 23 15:47:33 HST 2020",
  workbook: "Sun May 24 21:45:03 HST 2020"
};

/***/ }),

/***/ "./src/js/modules/_config/wbkey.js":
/*!*****************************************!*\
  !*** ./src/js/modules/_config/wbkey.js ***!
  \*****************************************/
/*! exports provided: getIndexFromLesson */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "getIndexFromLesson", function() { return getIndexFromLesson; });
/*
...
function getIndexFromLesson(lesson) {
  let code = wbl[lesson];
...
function textnp($el) {
  ...
  let prevSection = section; //** find next section **
  ...
  if (nextChapter === -1) {
    ...
    let nextText = $(`#c${nextChapter} a[data-secid="${nextSection}"]`).text(); //set next tooltip and href
  ...
  if (prevChapter === -1) {
    ...
    let prevText = $(`#c${prevChapter} a[data-secid="${prevSection}"]`).text(); //set prev tooltip and href
...
function loadTOC(toc) {
  ...
  Object(_config_config__WEBPACK_IMPORTED_MODULE_1__["getConfig"])(book).then(contents => {
    ...
    switch (contents.bid) {
      ...
      case "workbook":
        html = makeWorkbookContents(contents.contents); //$(".toc-list").html(makeWorkbookContents(contents.contents));

        break;

      case "manual":
      case "acq":
        html = makeManualContents(contents.base, contents.contents); //$(".toc-list").html(makeManualContents(contents.base, contents.contents));
      ...
      default:
        html = makeContents(contents.contents); //$(".toc-list").html(makeContents(contents.contents));
  ...
  initialize: function (env) {
    ...
    $(uiOpenTocModal).on("click", e => {
      ...
      if (env !== "transcript" || env === "transcript" && combined) {
        Object(_config_config__WEBPACK_IMPORTED_MODULE_1__["getConfig"])(book).then(contents => {
          $(".toc-image").attr("src", `${contents.image}`);
          $(".toc-title").html(`Table of Contents: <em>${contents.title}</em>`); //mark toc as combined
...
const page = __webpack_require__(/*! ../_config/key */ "./src/js/modules/_config/key.js"); //const queryResultName = "query-result-raj";
...
function search(query) {
  ...
  axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(searchEndpoint, searchBody).then(response => {
    ...
    Object(www_modules_audit_audit__WEBPACK_IMPORTED_MODULE_5__["searchAudit"])("ACIMOE", searchBody.query, response.data.count);
    document.getElementById("search-input-field").focus();
  }).catch(error => {
    console.error("search error: %o", error);
    displaySearchMessage(SEARCH_ERROR, error.message);
    Object(www_modules_audit_audit__WEBPACK_IMPORTED_MODULE_5__["searchAudit"])("ACIMOE", searchBody.query, 0, error.message);
  });
}
...
const keyInfo = __webpack_require__(/*! ../_config/key */ "./src/js/modules/_config/key.js");
...
function showSearchResults(data, query) {
  ...
  console.log("processing search results"); //get array of all unique page info - promises
...
function saveQueryResults(queryString, matchCount, titleArray, pageInfo, data, originalResult) {
  const books = keyInfo.getBooks();
  let keyLength = keyInfo.getKeyInfo().keyLength; //don't save if there were no matches
...
function pageDriver() {
  ...
  driver.defineSteps([cmiPageTitle, getAcquainted, acimPreface, acimText, acimWorkbook, acimManual]);
  driver.start();
}
