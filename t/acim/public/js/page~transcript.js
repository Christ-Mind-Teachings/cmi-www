...
const {
  getUrl: acim_getUrl
} = __webpack_require__(/*! acim/modules/_config/key */ "./src/js/modules/_config/key.js");
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  keyInfo: keyInfo,
  //list, bmnet
  bm_modal_key: "bm.acim.modal",
  //list
  bm_creation_state: "bm.acim.creation",
  //bookmark
  bm_list_store: "bm.acim.list",
  //bmnet
  bm_topic_list: "bm.acim.topics",
  //bmnet
  bm_modal_store: "bm.acim.modal",
  //navigator
...
const SOURCE = "A Course In Miracles"; //mp3 and audio timing base directories

const audioBase = `https://s3.amazonaws.com/${AWS_BUCKET}/${SOURCE_ID}/audio`;
const timingBase = "/t/acim/public/timing"; //location of configuration files

const configUrl = "/t/acim/public/config";
const configStore = "config.acim."; //the current configuration, initially null, assigned by getConfig()
...
const status = {
  acq: "Tue Mar 19 02:43:51 HST 2019",
  manual: "Tue Mar 19 02:43:51 HST 2019",
  preface: "Tue Mar 19 02:43:51 HST 2019",
  text: "Tue Jul 9 16:39:00 HST 2019",
  workbook: "Tue Dec 31 16:46:23 HST 2019",
  raj: "Tue Mar 19 02:43:51 HST 2019"
};
...
function textNexPrev($el) {
  ...
  let nextSection; //next, set chapter = -1 if new value is invalid
  ...
  if (nextChapter === -1) {
    ...
    let nextText = $(`a[data-secid="${nextSecid}"]`).text(); //set next tooltip and href
  ...
  if (prevChapter === -1) {
    ...
    let prevText = $(`a[data-secid="${prevSecid}"]`).text(); //set prev tooltip and href
...
function loadTOC(toc) {
  ...
  Object(_config_config__WEBPACK_IMPORTED_MODULE_1__["getConfig"])(book).then(contents => {
    ...
    switch (contents.bid) {
      case "text":
        html = makeTextContents(contents.contents); //$(".toc-list").html(makeTextContents(contents.contents));
...
function search(query) {
  ...
  axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(searchEndpoint, searchBody).then(response => {
    ...
    Object(www_modules_audit_audit__WEBPACK_IMPORTED_MODULE_5__["searchAudit"])("ACIM", searchBody.query, response.data.count);
    document.getElementById("search-input-field").focus();
  }).catch(error => {
    console.error("search error: %o", error);
    displaySearchMessage(SEARCH_ERROR, error.message);
    Object(www_modules_audit_audit__WEBPACK_IMPORTED_MODULE_5__["searchAudit"])("ACIM", searchBody.query, 0, error.message);
  });
}
